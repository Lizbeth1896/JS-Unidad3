...
function new_user1() {
    let user = prompt("Ingrese la informacion del usuario (nombre, apellido, edad, profesion)")
    user = user.split(", ")
    let id = users.length + 1
    users.push(
    {id: id, nombre: user[0], apellido: user[1], edad: user[2], profesion: user[3]}
    )
    console.log(users)
}
...
function READ() {
    ...
    for(var i=0;i<users.length;i++){
        ...
        for	(var j=0;users[i].length;j++){
            var celda=document.createElement("td");


            var id=document.createTextNode(document.getElementsByTagName("td")[j].innerHTML=users[j].id);
            var nombre=document.createTextNode(document.getElementsByTagName("td")[j].innerHTML=users[j].nombre);
            var apellido=document.createTextNode(document.getElementsByTagName("td")[j].innerHTML=users[j].apellido);
            var edad=document.createTextNode(document.getElementsByTagName("td")[j].innerHTML=users[j].edad);
            var profesion=document.createTextNode(document.getElementsByTagName("td")[j].innerHTML=users[j].profesion);
            var created_at=document.createTextNode(document.getElementsByTagName("td")[j].innerHTML=users[j].created_at);
            

            var id=fila.appendChild(celda.appendChild(id));
            var nombre=fila.appendChild(celda.appendChild(nombre));
            var apellido=fila.appendChild(celda.appendChild(apellido));
            var edad=fila.appendChild(celda.appendChild(edad));
            var profesion=fila.appendChild(celda.appendChild(profesion));
            var created_at=fila.appendChild(celda.appendChild(created_at));
